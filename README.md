...
**Integration** — a markdown vault that several AI agents can read, write and maintain together without treading on each other.
...
funnel, the queue, the record format, the synthesis layer, the health check and
the cost ledger all run.
...
and a local vision model, and that is the surface where both real leaks
actually lived.
...
Documentation standard for this and sibling repos: `docs/product-readme-standard.md`.
